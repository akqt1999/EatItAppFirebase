...
public interface CartDAO {
    ...
    @Query("SELECT SUM ((foodExtraPrice+foodPrice)*foodQuantity) FROM Cart WHERE uid=:uid")// @Query("SELECT SUM((foodPrice+foodExtraPrice))*foodQuantity FROM Cart WHERE uid=:uid")
    ...
    @Query("SELECT foodName FROM Cart WHERE uid=:uid")
    Single<String>getFoodId(String uid);


    // cai nay co the de category nhưng minh khong thích để tai vi de no loi
